...
public class HelpScreen extends ScreenAdapter {
	...
	public HelpScreen(MyGdxGame g) {
		...
		exp1 = new Vector2(width, height);
		exp2 = new Vector2(width, height);
		exp3 = new Vector2(width, height);
		exp4 = new Vector2(width, height);
		pos0 = new Vector2(width, height);
		pos1 = new Vector2(width, height);
		pos2 = new Vector2(width, height);
		pos3 = new Vector2(width, height);
		pos4 = new Vector2(width, height);
		pos5 = new Vector2(width, height);
		pos6 = new Vector2(width, height);
		pos7 = new Vector2(width, height);
		pos8 = new Vector2(width, height);
	...
	public void render(float delta) {
		...
		for (int i = 0; i < tasklist.size() / 2; i++) {
			if (tasklist.get(i).isCompleted() && changePic[i]) {
				...
				batch.draw(tasklist.get(i).getCompletedPic(), (i + 1)
						* (width * .15f), height * .8f, tasklist.get(i)
						.getUncompletedPic().getWidth(), tasklist.get(i)
						.getUncompletedPic().getHeight());
			} else {
				font.setColor(Color.LIGHT_GRAY);
				batch.draw(tasklist.get(i).getUncompletedPic(), (i + 1)
						* (width * .15f), height * .8f);
			}
		}
		for (int i = tasklist.size() / 2; i < tasklist.size(); i++) {
			if (tasklist.get(i).isCompleted() && changePic[i]) {
				font.setColor(Color.BLACK);
				batch.draw(tasklist.get(i).getCompletedPic(),
						(i + 1 - tasklist.size() / 2) * (width * .15f),
						height * .6f);
			} else {
				font.setColor(Color.LIGHT_GRAY);
				batch.draw(tasklist.get(i).getUncompletedPic(),
						(i + 1 - tasklist.size() / 2) * (width * .15f),
						height * .6f);
			}
		}
		...
		for (int i = 0; i < tasklist.size(); i++) {
			if (tasklist.get(i).isCompleted()) {
				...
				if (vecArr.get(i).x <= width * .073)
					font.setColor(Color.BLACK);
				font.draw(batch, "Task " + (i + 1) + ": "
						+ tasklist.get(i).getDescription(), width * .1f, height
						* .6f - height * .05f * (i + 1));
	...
	public void starExplosion(float x, float y, int index) {
		...
		if (exp1.y < height + 1000 && exp2.y < height + 1000 && exp3.y > -1000
				&& exp4.y > -1000) {
