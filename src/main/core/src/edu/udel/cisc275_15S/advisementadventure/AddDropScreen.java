...
public class AddDropScreen extends ScreenAdapter {
	...
	public AddDropScreen(MyGdxGame g) {
		...
		// Create a list of items that stores the possible classes that the user
		// can take
		...
		// initializes a drop items list for the possible classes that the user
		// can drop
		...
		add.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent e, float x, float y,
				...
				if (sb.getSelected() != null && sb.getSelected() != "") {
					...
					// displays a message to the user confirming that they have
					// added the class
					AddL.setText("Congrats! You have added: "
							+ sb.getSelected());
					s.addActor(AddL);
		...
		drop.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent e, float x, float y,
				...
				if (sb2.getSelected() != null && sb2.getSelected() != "") {
					...
					// displays a message to the user confirming that they have
					// dropped the class
					DropL.setText("Congrats! You have dropped: "
							+ sb2.getSelected());
					s.addActor(DropL);
	...
	// creates a back button that the user is able to click on in order to go
	// back to the udsis screen
	...
	public void render(float delta) {
		...
		batch.draw(registration, 25, height - (2 * (height / 5.64f)), width,
				height / 6.5f);
		batch.end();
		s.act(Math.min(Gdx.graphics.getDeltaTime(), 1 / 30f));
		s.draw();
